...
pub struct SkillTool {
    ...
    /// seconds before the command's process group is killed. Unset falls back
    /// to the built-in `SKILL_SHELL_TIMEOUT_SECS` (60s) default; long-running
    /// skills (e.g. a build pipeline) raise it via `timeout_secs` in SKILL.toml.
    #[serde(default)]
