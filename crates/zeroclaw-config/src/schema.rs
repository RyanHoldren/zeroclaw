...
pub struct GatewayConfig {
    ...
    /// Send WebSocket ping frames every N seconds. 0 disables pings.
    /// The value is read when each WebSocket connection opens.
...
mod tests {
    ...
    async fn validate_accepts_websocket_ping_interval_upper_bound() {
        ...
        config.validate().expect("upper bound must validate");
    }
